'use client'

import React, { useState } from 'react'
import Image from 'next/image'
import { Dumbbell, ChevronLeft, ChevronRight, Sparkles } from 'lucide-react'

const fitnessImages = [
  {
    src: '/mother_fitness_program/image.png',
    title: 'Energizing Zumba & Dance Cardio',
    caption: 'High-energy beats, calorie burn, and joyful moves with fellow Patna moms',
    tag: 'Zumba Cardio'
  },
  {
    src: '/mother_fitness_program/image copy.png',
    title: 'Postnatal Core & Pelvic Floor Yoga',
    caption: 'Gentle rehab techniques designed to heal diastasis recti and rebuild core strength',
    tag: 'Postnatal Yoga'
  },
  {
    src: '/mother_fitness_program/image copy 2.png',
    title: 'Functional Mom-Strength Training',
    caption: 'Guided dumbbell and resistance workouts for posture and back pain relief',
    tag: 'Strength & Tone'
  },
  {
    src: '/mother_fitness_program/image copy 3.png',
    title: 'Supportive Community of Moms',
    caption: 'Workout alongside friendly mothers in a warm, judgment-free environment',
    tag: 'Mom Tribe'
  },
  {
    src: '/mother_fitness_program/image copy 4.png',
    title: 'Breathwork & Guided Stretching',
    caption: 'Relaxing cooldown sequences to relieve neck, shoulder, and lower back tension',
    tag: 'Holistic Wellness'
  },
  {
    src: '/mother_fitness_program/image copy 5.png',
    title: 'Aerobics & Stamina Building',
    caption: 'Boost everyday energy levels to keep up with active toddlers',
    tag: 'Aerobics'
  },
  {
    src: '/mother_fitness_program/image copy 6.png',
    title: 'Posture Correction & Flexibility',
    caption: 'Targeted exercises for nursing mothers and postpartum body recovery',
    tag: 'Flexibility'
  },
  {
    src: '/mother_fitness_program/image copy 7.png',
    title: 'Personalized Attention from Trainers',
    caption: 'Certified female coaches adjusting routines for each mother’s fitness level',
    tag: 'Expert Guidance'
  }
]

export default function FitnessGallery() {
  const [currentIndex, setCurrentIndex] = useState(0)

  const prevSlide = () => {
    setCurrentIndex((prev) => (prev === 0 ? fitnessImages.length - 1 : prev - 1))
  }

  const nextSlide = () => {
    setCurrentIndex((prev) => (prev === fitnessImages.length - 1 ? 0 : prev + 1))
  }

  return (
    <div className="w-full bg-white rounded-3xl p-6 sm:p-8 shadow-xl border border-emerald-100">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 font-bold text-xs mb-2">
            <Dumbbell className="w-3.5 h-3.5" />
            <span>Phulwari Fitness Moments</span>
          </div>
          <h3 className="text-2xl font-black text-slate-800">Peek Inside Our Mothers’ Fitness Sessions</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={prevSlide}
            aria-label="Previous photo"
            className="w-10 h-10 rounded-full bg-slate-100 hover:bg-emerald-600 hover:text-white flex items-center justify-center transition shadow-sm"
          >
            <ChevronLeft className="w-5 h-5" />
          </button>
          <span className="text-xs font-bold text-slate-500 px-2">
            {currentIndex + 1} / {fitnessImages.length}
          </span>
          <button
            onClick={nextSlide}
            aria-label="Next photo"
            className="w-10 h-10 rounded-full bg-slate-100 hover:bg-emerald-600 hover:text-white flex items-center justify-center transition shadow-sm"
          >
            <ChevronRight className="w-5 h-5" />
          </button>
        </div>
      </div>

      {/* Main Feature Image Display */}
      <div className="relative w-full h-[320px] sm:h-[450px] lg:h-[480px] rounded-2xl overflow-hidden bg-slate-100 shadow-inner group">
        <Image
          src={fitnessImages[currentIndex].src}
          alt={fitnessImages[currentIndex].title}
          fill
          priority
          sizes="(max-width: 768px) 100vw, (max-width: 1200px) 80vw, 1000px"
          className="object-cover transition-transform duration-700 group-hover:scale-105"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/20 to-transparent pointer-events-none" />

        <div className="absolute top-4 left-4">
          <span className="bg-emerald-600 text-white font-extrabold text-xs px-3 py-1.5 rounded-full shadow-lg">
            {fitnessImages[currentIndex].tag}
          </span>
        </div>

        <div className="absolute bottom-6 left-6 right-6 text-white pointer-events-none">
          <h4 className="text-xl sm:text-2xl font-black tracking-tight drop-shadow-md mb-1">
            {fitnessImages[currentIndex].title}
          </h4>
          <p className="text-xs sm:text-sm text-emerald-100 font-medium drop-shadow leading-relaxed max-w-xl">
            {fitnessImages[currentIndex].caption}
          </p>
        </div>
      </div>

      {/* Thumbnails Row */}
      <div className="grid grid-cols-4 sm:grid-cols-8 gap-2 sm:gap-3 mt-6">
        {fitnessImages.map((img, idx) => (
          <button
            key={idx}
            onClick={() => setCurrentIndex(idx)}
            className={`relative h-16 sm:h-18 rounded-xl overflow-hidden transition-all duration-300 ${
              currentIndex === idx
                ? 'ring-3 ring-emerald-500 scale-105 shadow-md'
                : 'opacity-65 hover:opacity-100'
            }`}
          >
            <Image
              src={img.src}
              alt={img.title}
              fill
              sizes="120px"
              className="object-cover"
            />
            <div className="absolute inset-0 bg-black/20 hover:bg-transparent" />
          </button>
        ))}
      </div>
    </div>
  )
}
